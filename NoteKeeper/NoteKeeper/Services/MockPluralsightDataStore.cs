...
namespace NoteKeeper.Services
{
    public class MockPluralsightDataStore : IPluralsightDataStore
    {
        ...
        public async Task<bool> DeleteNoteAsync(Note courseNote)
        {
            var noteIndex = mockNotes.FindIndex((Note arg) => arg.Id == courseNote.Id);
            var deleteNoteFound = noteIndex != -1;
            if (deleteNoteFound)
            {
                mockNotes.RemoveAt(noteIndex);
            }
            return await Task.FromResult(deleteNoteFound);
        }
